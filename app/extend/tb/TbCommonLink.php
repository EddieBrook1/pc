...
class TbCommonLink {
    ...
    public function handle ($cookie) {
        ...
        if (!$isOk) {
            ...
            RecordError::handle(__CLASS__, "淘宝接口http请求错误, 链接($url)");
            return Util::formatResult([], false, 'SYSTEM_ERROR');
        }


        // 淘宝接口返回异常, 提前返回
        if ($this->isApiOk($raw, $jsonp_callback_name) !== true) {

            RecordError::handle(__CLASS__, '淘宝接口返回异常, 异常类型无法判断');
        ...
        try {
            ...
            RecordError::handle(__CLASS__, '淘宝接口返回数据格式改变');
            $result_data = null;
        }
